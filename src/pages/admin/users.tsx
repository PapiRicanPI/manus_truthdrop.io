  ...
  async function changeRole(userId: string, newRole: string) {
    ...
    try {
      const res = await fetch("/api/admin/users", {
        ...
        body: JSON.stringify({ id: userId, assignedRole: newRole }),
      });
      if (res.ok) {
        setUsers((prev) => prev.map((u) => u.id === userId ? { ...u, assignedRole: newRole } : u));
    ...
    <>
      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .header { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem; }
        h1 { font-size: 1.5rem; font-weight: 700; color: #f1f5f9; }
        .subtitle { color: #64748b; font-size: 0.875rem; margin-top: 0.25rem; }
        .legend { display: flex; gap: 1.25rem; flex-wrap: wrap; margin-bottom: 1.5rem; padding: 1rem; background: #0d0d17; border: 1px solid #1e293b; border-radius: 8px; }
        ...
        .count { background: #1e293b; color: #94a3b8; font-size: 0.75rem; padding: 0.2rem 0.6rem; border-radius: 999px; margin-left: 0.5rem; vertical-align: middle; }
      `}</style>
      <div className="wrap">
        ...
        <div className="header">
          <div>
            ...
            <p className="subtitle">All approved users and their access levels. Use the dropdown to change a role.</p>
          </div>
        </div>
        ...
        <div className="legend">
          {ROLE_OPTIONS.map((r) => (
            <div key={r.value} className="legend-item">
              ...
              <span className="legend-role" style={{ color: r.color }}>{r.label}</span>
              <span className="legend-desc">—
                {r.value === "observer" && " Read-only access to published cases"}
                {r.value === "researcher" && " Case & evidence work"}
                {r.value === "custodian" && " Tip review, redaction, case linking"}
                {r.value === "admin" && " Full settings & role management"}
              </span>
            </div>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <table>
            ...
            <tbody>
              {users.map((u) => {
                const meta = roleMeta(u.assignedRole);
                return (
                  <tr key={u.id}>
                    ...
                    <td style={{ color: "#64748b", fontSize: "0.8rem" }}>{formatDate(u.approvedAt)}</td>
                    <td>
                      {u.id === "admin" ? (
